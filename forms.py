...
class RepositoryForm(FlaskForm):
    ...
    def validate_name(self, name):
        ...
        if self.repo_id.data and str(Repository.query.get(self.repo_id.data).id) == self.repo_id.data:
            return
        ...
        if repo:
            raise ValidationError('You already have a repository with this name. Please choose a different name.')
    ...
    def validate_repo_url(self, repo_url):
        ...
        if repo:
            raise ValidationError('A repository with this URL already exists in your account.')
...
class JournalEntryForm(FlaskForm):
    ...
    title = StringField('Title', validators=[
        DataRequired(),
        Length(min=2, max=200, message='Title must be between 2 and 200 characters long')
    ])
    content = TextAreaField('Content', validators=[
        DataRequired(message='Please enter some content for your journal entry'),
        Length(min=10, message='Your journal entry should be at least 10 characters long')
    ])
    ...
    def validate_content(self, content):
        ...
        if len(content.data.strip()) < 10:
            raise ValidationError('Your journal entry seems too short. Please provide more details.')
